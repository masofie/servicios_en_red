# Instalación y Configuración Servicio HTTP/HTTPS

1. [Recursos con el Protocolo HTTP y HTTPS](1-recuros-protocolo-http.md)
2. [Instalación y Configuración de Apache en XAMMP](2-instalacion-configuracion-apache-windows.md)
3. [Despligue de Tromcat en XAMPP](3-despligue-tromcat.md)
4. [Instalación y Configuración del Servidor ISS en Windows ](4_instalacion_iss_ws.md)
5. [Creación de Sitios Virtuales en Apache2 en Debian](5_creacion_sitios_apache2.md)
6. [Alta Disponibilidad con HAProxy en Debian](6_HAProxy.md)
